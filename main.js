...
const REGISTRATION_EXPORT_I18N = {
  id: {
    title: 'Data Pendaftaran',
    submittedAt: 'Tanggal',
    empty: 'Belum ada data pendaftaran.'
  },
  en: {
...
function resolveRegistrationLabel(field, language) {
  ...
  if (field.labelKey && labels[language]?.[field.labelKey]) return labels[language][field.labelKey];
  return field.labelKey || 'Field';
}

function buildRegistrationPdfHtml({ schema = [], submissions = [], language = 'id' }) {
  const i18n = REGISTRATION_EXPORT_I18N[language] || REGISTRATION_EXPORT_I18N.id;
  const headerCells = [
    `<th>${escapeHtml(i18n.submittedAt)}</th>`,
    ...schema.map((field) => `<th>${escapeHtml(resolveRegistrationLabel(field, language))}</th>`)
  ].join('');
  const rows = submissions
    .map((entry) => {
      const created = new Date(entry.submitted_at || new Date());
      const dateLabel = created.toLocaleString(language === 'en' ? 'en-US' : 'id-ID');
      const values = schema.map((field) => escapeHtml(entry.values?.[field.id] || '-'));
      return `<tr><td>${escapeHtml(dateLabel)}</td>${values
        .map((value) => `<td>${value}</td>`)
        .join('')}</tr>`;
    })
    .join('');
  const body = rows || `<tr><td colspan="${schema.length + 1}">${escapeHtml(i18n.empty)}</td></tr>`;
  return `
  <!DOCTYPE html>
  <html lang="${language}">
  <head>
...
function buildRegistrationExcelHtml({ schema = [], submissions = [], language = 'id' }) {
  const i18n = REGISTRATION_EXPORT_I18N[language] || REGISTRATION_EXPORT_I18N.id;
  const headerCells = [
    `<th>${escapeHtml(i18n.submittedAt)}</th>`,
    ...schema.map((field) => `<th>${escapeHtml(resolveRegistrationLabel(field, language))}</th>`)
...
ipcHandle('add-task', async (event, payload) => {
  const result = await taskService.addTask(payload || {});
  if (!result) {
...
ipcHandle('update-task', async (event, { id, expected_updated_at, ...payload }) => {
  const result = await taskService.updateTask(id, payload || {}, expected_updated_at || null);
  if (!result) {
...
ipcHandle('get-projects', async () => {
  return await queries.getProjects();
});

ipcHandle('get-clients', async () => {
  return await queries.getClients();
});

ipcHandle('get-users', async () => {
  return await queries.getUsers();
});

ipcHandle('get-workspaces', async () => {
  return await queries.getWorkspaces();
});
...
ipcHandle('get-runtime-info', async () => {
  return {
...
ipcHandle('export-registration-pdf', async (event, payload) => {
  const { canceled, filePath } = await dialog.showSaveDialog({
    title: 'Export Registration PDF',
    defaultPath: 'streakly-registration.pdf',
    filters: [{ name: 'PDF', extensions: ['pdf'] }]
  });
  if (canceled || !filePath) return false;
  const html = buildRegistrationPdfHtml(payload || {});
  const reportWindow = new BrowserWindow({
    show: false,
    webPreferences: {
      contextIsolation: true,
...
ipcHandle('export-registration-excel', async (event, payload) => {
  const { canceled, filePath } = await dialog.showSaveDialog({
    title: 'Export Registration Excel',
    defaultPath: 'streakly-registration.xls',
    filters: [{ name: 'Excel', extensions: ['xls'] }]
  });
  if (canceled || !filePath) return false;
  const html = buildRegistrationExcelHtml(payload || {});
